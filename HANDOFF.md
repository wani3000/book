...
현재 이 Mac의 작업 저장소는 `/Users/chulwan/Documents/Codex/2026-07-18/pc/website`이며 GitHub `wani3000/book`의 `main` 브랜치를 사용한다. 다른 PC나 에이전트에서 이어갈 때는 `git rev-parse --show-toplevel`로 실제 경로를 확인하고, 먼저 `git pull --ff-only origin main`을 실행한 뒤 이 문서와 `design-qa.md`를 읽는다.
...
- 최신 검증 결과: ESLint 통과, 프로덕션 빌드 성공, 자동 테스트 26개 통과, 환불정책 브라우저 콘솔 오류 0건, 로컬 D1 환불 통합 흐름 통과.
- 회원 동의 필드를 추가한 D1 migration은 `drizzle/0006_typical_metal_master.sql`이며 다음 배포에 반드시 포함한다.
...
| --- | --- | --- | ---: | ---: | --- |
| `codex` | 아이디어를 서비스로 바꾸는 Codex 사용법 | 필립 | 233쪽 | 19,000원 | `/codex` |
| `career` | 커리어도 디자인할 수 있습니다 | 필립 | 77쪽 | 19,000원 | `/career` |
...
### P0 — 다음 공개 배포에서 D1 migration 0004 적용 필수

이번 변경은 `orders.first_accessed_at`과 `refund_requests` 테이블을 추가한다. `drizzle/0004_wonderful_tenebrous.sql`이 배포 아카이브의 `dist/.openai/drizzle/`에 포함되는지 반드시 확인하고 Sites 버전을 저장해야 한다. migration 없이 코드만 배포하면 구매 내역과 PDF 열람 API가 데이터베이스 오류를 반환한다.
...
5. 배포 아카이브 생성
6. `dist/.openai/drizzle/0004_wonderful_tenebrous.sql` 포함 확인
7. 새 사이트 버전 저장
