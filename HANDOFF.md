...
- 마이페이지는 현재 `/mypage#overview`, `#orders`, `#profile` 해시와 로컬 상태를 사용한다. 사용자 승인 구조는 실제 `/mypage`, `/mypage/orders`, `/mypage/profile` 페이지 이동이므로 현 구현은 출시 차단 결함이다.
- 모바일·태블릿에서는 사이드바 다음에 선택 콘텐츠가 놓여 “메뉴 아래에 내용이 쌓이는” 문제가 재현된다. 다음 작업에서 실제 하위 라우트와 공통 레이아웃으로 분리해야 한다.
...
실제 경험을 기반으로 만든 전자책 3권을 제작하고, 클래스101과 유사한 탐색·상세·구매 흐름을 가진 판매 사이트 `다니엘의 노트`에서 판매한다. 영문 워드마크는 `DANIEL'S NOTE`다.
...
- 마이페이지 메뉴는 아직 해시 기반이며 `/mypage/orders`, `/mypage/profile`이 존재하지 않는다. 인증된 QA 보고서 `audit/mypage-journey-qa-2026-07-21-authenticated/REPORT.md` 판정은 `BLOCK`이다.
