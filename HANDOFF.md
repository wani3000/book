...
- 카카오페이·네이버페이 심사 준비 변경은 최신 Sites 버전으로 공개 배포하며, 정확한 버전은 Sites와 Git 로그를 기준으로 확인
...
카카오페이 심사 준비 구현 커밋은 `f7c20ae13221170bc8ac7eb4b0005143309b05be`, 배포 호환성 수정 커밋은 `e6cce6328a6c7a436a65b1fe1e1c2ba762b09538`이다. 네이버페이 추가 이후 기준은 Git 로그의 최신 커밋을 확인한다.
...
Paddle 결제 코드는 보조·롤백 경로로 저장소에 남아 있지만 국내 판매용 운영 결제로 활성화하지 않는다. 카카오페이 운영 승인이 완료되면 카카오페이를 우선 사용한다. 현재 공개 사이트의 구매 버튼은 카카오페이 운영 키가 연결되기 전까지 `카카오페이 심사 준비 중`으로 비활성화된다. 비밀값은 저장소나 인계 문서에 쓰지 말고 Sites 런타임 설정에만 저장한다.
...
2026-07-20에 1440px 데스크톱 랜딩·상세 3종과 390px 모바일 랜딩·상세 화면을 검수했다. 기록은 `/Users/chulwan/Documents/Codex/2026-07-18/pc/audit/final-launch-2026-07-20/`에 저장한다. PDF 3권은 페이지 수, 빈 본문, 깨진 글자와 앞·중간·마지막 대표 페이지 렌더링을 확인했다.
