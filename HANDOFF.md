...
- Google URL 검사에서 메인과 전자책 상세 3종(`/`, `/codex`, `/career`, `/jane`)이 아직 미등록 상태임을 확인하고 2026-07-23 우선순위 크롤링 대기열에 각각 추가했다.
- 사이트맵의 보조 공개 페이지(`/payment`, `/account-guide`, `/terms`, `/privacy`, `/refund`)가 메인을 canonical로 가리키던 문제를 수정해 각 페이지가 자신의 URL을 canonical로 선언한다.
- 네이버 서치어드바이저 소유확인을 완료하고 `https://danielsnote.com/sitemap.xml`을 2026-07-23 제출했다.
- Google 운영 계정의 2단계 인증과 복구 수단 등록을 확인했다. 카카오 운영 계정도 2026-07-23 로그인 후 `2단계 인증 · 카카오톡 인증 · 안전` 상태를 확인했다.
- 공통 헤더는 데스크톱에서도 로고 왼쪽에 전자책 카테고리 메뉴 아이콘을 표시한다. 데스크톱 프로필 왼쪽의 중복 `전체 전자책` 링크는 제거했고, 메뉴는 420px 좌측 서랍으로 열린다.
- Google Analytics에 `다니엘의 노트` GA4 속성과 웹 데이터 스트림을 만들고 Sites 환경변수에 측정값을 연결했다. 사이트는 방문자가 익명 분석에 동의한 경우에만 태그를 불러오며, 거절해도 모든 기능을 이용할 수 있다. 공개 사이트에서 동의 전 태그 0개, 동의 후 태그 1개와 안내 패널 종료를 확인했다. 동의한 방문자에게만 `page_view`, `view_item`, `begin_checkout`, 결제 주문 검증 후 `purchase` 이벤트를 전송한다.
...
- 최신 검증 결과: ESLint 통과, 프로덕션 빌드 성공, 자동 테스트 45개 통과, QA 관리자 인증 스모크 테스트 통과.
- migration `0006`~`0009`는 운영에 적용됐으며 `/api/health`에서 schema `0009`를 확인했다. 이후 배포도 전체 migration 파일을 아카이브에 포함한다.
...
- 로그인 제공자 분리와 기존 Google 회원 자동 이관 migration은 `drizzle/0007_icy_silver_surfer.sql`이며 운영 적용을 확인했다.
...
### 사용자 진행 — 카카오페이 가맹 신청과 운영 키 발급

카카오페이 가맹 신청·본인 인증·계약 제출은 사용자가 직접 마무리한다. 에이전트는 사용자의 별도 요청 없이 신청 화면을 조작하거나 제출하지 않는다.

공개 고객센터 번호는 사이트와 Sites 환경변수에 반영했다. 카카오 계정 로그인 후 온라인 결제·독립형 쇼핑몰을 선택해 1차 신청 조건 확인 화면까지 진입했다. 정산 계좌와 KYC 서류는 외부 신청 과정에서 사용자가 직접 확인해야 한다.
...
### 사용자 진행 — 네이버페이 결제형 가맹 신청과 인증값 발급

네이버페이 가맹 신청·휴대전화 인증·계약 제출은 사용자가 직접 마무리한다. 에이전트는 사용자의 별도 요청 없이 신청 화면을 조작하거나 제출하지 않는다.
...
### 후기 운영 절차 — 자동 구매 검증 적용 완료

`POST /api/reviews`는 로그인 회원의 결제 완료 주문을 서버에서 자동 확인해 상품별 후기와 연결한다. 회원당 상품별 1개로 제한하며 다시 제출하면 수정한다. 관리자는 `/admin/reviews`에서 내용을 검토해 승인 또는 비공개 처리하고, 승인된 구매 인증 후기만 공개한다.

### D1 migration — 운영 적용 완료

회원 약관 동의, 로그인 제공자 분리, 출시 전 운영 보강, 친절한 이메일 여정 migration `0006`~`0009`가 운영에 적용됐다. 배포 후 `/api/health`의 schema가 `0009`인지 확인하고, 모든 배포 아카이브에 전체 migration 파일이 포함됐는지 계속 점검한다.
...
5. 배포 아카이브 생성
6. `dist/.openai/drizzle/`에 migration `0006`~`0009` 포함 확인
7. 새 사이트 버전 저장
...
1. 사용자가 카카오페이·네이버페이 가맹 신청과 계약 제출을 마무리한다. 에이전트는 별도 요청 없이 신청 화면을 조작하지 않는다.
2. 승인 후 발급된 운영 인증값을 Git이나 문서가 아닌 Sites 비밀 환경변수에만 연결하고 각 결제수단을 개별 활성화한다.
3. 테스트 결제 완료·사용자 취소·결제 실패·환불을 실행해 주문 적재, 메일, PDF 권한 부여·회수를 검증한다.
4. 실제 운영 결제로 환불을 한 번 승인해 결제 취소 성공, 주문 `refunded`, PDF 접근 403, 마이페이지 `환불 완료`를 확인한다.
5. 가맹 승인 후 모바일·데스크톱에서 실제 결제수단 버튼과 외부 결제창을 다시 시각 QA한다.
6. 네이버 서치어드바이저 보안문자를 사용자가 직접 확인한 뒤 소유 확인과 `/sitemap.xml` 제출을 완료한다.
7. GA4 실시간·DebugView에서 `page_view`, `view_item`, `begin_checkout`, `purchase` 이벤트와 주문번호 중복 방지를 확인한다.
...
- 첫 자동화 브라우저 요청에서는 Cloudflare JavaScript Detection이 HTML에 삽입한 태그 때문에 React recoverable hydration 오류가 한 번 기록될 수 있다. 화면과 기능은 복구된다. 삽입 태그를 앱에서 강제로 제거하면 상세 페이지가 깨지므로 제거하지 않으며, 필요하면 Cloudflare/Sites 플랫폼의 봇 감지 설정에서 해결한다.
...
- GA4는 방문자 동의 후에만 페이지·상품 조회·결제 시작·검증된 구매 이벤트를 전송한다. 광고 태그는 연결하지 않았다.
