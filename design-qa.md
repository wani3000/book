...
- 로그인 상태 헤더에는 `로그인` 대신 해당 계정의 표시 이름이 노출된다.
...
- State: Google 테스트 계정 로그인 완료, 전자책 3권 구매 권한.
...
final result: passed
# 2026-07-22 웹 타이포그래피 전수 QA

- 범위: 메인, 전자책 3권 상세, 로그인·마이페이지, 정책 5종, 결제 성공·실패, QA 로그인, 관리자 경로.
- 원칙: Pretendard 400/700만 사용하고 일반 본문은 16px, 데스크톱·태블릿은 최소 14px, 모바일은 최소 15px로 유지한다. 버튼 라벨은 별도 토큰인 13px Regular 400으로 통일하며 버튼 높이와 클릭 영역은 유지한다.
- 제외: PDF 3권과 `BusinessFooter`는 변경하지 않았다.
