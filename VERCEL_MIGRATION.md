...
- GitHub `main` 공개 이력을 개인정보 없는 새 기준점부터 다시 시작하도록 재작성하고 과거 공개 작업 브랜치 삭제

## 남은 외부 단계

1. GA4 실시간 보고서의 첫 데이터 반영을 확인한다.
2. 카카오페이·Npay 승인 후에만 각 운영 키를 등록하고 활성화 플래그를 켠다.
3. Vercel 무료 플랜의 일일 배포 제한이 해제되면 문서 전용 최신 커밋을 다시 배포한다.
4. GitHub Support에 병합 PR 1~4의 과거 민감정보 커밋 참조와 캐시 제거를 요청하거나 저장소 재생성 여부를 결정한다.
