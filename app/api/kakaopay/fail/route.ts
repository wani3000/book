...
import { notifyPaymentFailed } from "@/app/notifications/events";
...
export async function GET(request: Request) {
  ...
  if (member && orderId) {
    ...
    if (attempt?.status === "ready" && isEbookProduct(attempt.product)) await notifyPaymentFailed(member, { orderId, title: ebookCatalog[attempt.product].title });
  }
