...
export default function AccountGuidePage() {
  return <PolicyPage eyebrow="ACCOUNT GUIDE" title="회원·구매내역 안내">
    <section><h2>회원가입과 로그인</h2><p>다니엘의 노트는 별도 비밀번호 없이 Google 또는 카카오 계정으로 로그인합니다. 처음 이용하는 경우 소셜 계정 인증 후 이용약관과 개인정보 수집·이용에 필수로 동의해야 회원가입이 완료됩니다. 마케팅 수신 동의는 선택이며, 동의하지 않아도 구매와 전자책 이용에 제한이 없습니다.</p></section>
    ...
    <section><h2>로그인 계정 연결</h2><p>마이페이지의 프로필 관리에서 카카오 계정을 기존 회원에 연결할 수 있습니다. 이메일이 같다는 이유만으로 두 회원을 자동 병합하지 않으며, 기존 로그인으로 본인 확인을 마친 상태에서만 연결합니다. 로그인 수단을 추가해도 구매 내역과 PDF 권한은 하나의 회원 계정에 유지됩니다.</p></section>
    <section><h2>탈퇴 후 기록 보관과 재가입</h2><p>회원 프로필은 탈퇴 처리되지만 주문·결제·환불 등 거래 기록은 관계 법령이 정한 기간 동안 분리 보관합니다. 연결했던 소셜 계정으로 다시 로그인해도 자동 복구하지 않고 재가입 안내를 표시합니다. 필수 동의를 다시 확인하고 재가입하면 보관된 기존 구매 내역과 유효한 전자책 열람 권한이 다시 연결됩니다. 카카오 가입자는 탈퇴 처리와 함께 카카오 플랫폼 연결 해제를 요청합니다.</p></section>
