...
export default function PolicyPage({ eyebrow, title, children }: { eyebrow: string; title: string; children: React.ReactNode }) {
  return <main className="policy-site">
    <header className="policy-header"><Link href="/">DANIEL&apos;S NOTE</Link><nav><Link href="/payment">결제·이용</Link><Link href="/terms">이용약관</Link><Link href="/privacy">개인정보처리방침</Link><Link href="/refund">교환·환불정책</Link></nav></header>
